...
static int highlighted(int x, int y) {
  int const s = term.row * term.col, i = y * term.col + x,
            sz = size(&searchStr);
...
static void markSearchMatches(int all) {
  ...
  for (int y = 0, wi = 0, owi = 0, i = 0; sz && y < term.row; ++y)
    for (int x = 0; x < term.col; ++x, wi %= sz, ++i, owi = wi)
      if (all || term.dirty[y]) {
        mark[i] =
            sz - (wi = (getU32(&searchStr, wi, 1) == term.line[y][x].u ? wi + 1
                                                                       : 0));
        if (wi == 1)
          ox = x, oy = y, oi = i;
        else if (!wi && owi)
...
static int findString(int s, int all) {
  Pos p =
      (Pos){.p = {term.c.x, term.c.y, IS_SET(MODE_ALTSCREEN) ? 0 : histOff}};
  historyMove(s, 0, 0);
  uint32_t strSz = size(&searchStr), maxIter = rows() * term.col + strSz,
           wIdx = 0;
  for (uint32_t i = 0, wi = 0; wIdx < strSz && ++i <= maxIter;
...
/// Expand "infix" expression: for instance (w =>)       l     b     |   | v e
/// |   | y
static ExitState expandExpression(
    char l) { //    ({ =>)       l  ?  {  \n | l | v  /  } \n | h | y
  int a = state.cmd.infix == infix_a, yank = state.cmd.op == 'y',
      lc = tolower(l), found = 1;
  ...
  if (lc == 'w')
    ...
    mot[1] = '?', mot[3] = mot[8] = '\n', mot[6] = '/',
    mot[4] = (char)(a ? 0 : 'l'), mot[9] = (char)(a ? 0 : 'h');
...
ExitState kPressHist(char const *cs, size_t len, int ctrl, KeySym const *kSym) {
  ...
  if (esc || ret || ins) {
  ...
  } else if (state.cmd.infix && state.cmd.op &&
             (result = expandExpression(cs[0]), len = 0)) {
  } else if (cs[0] == 'd') {
    ...
    if (state.cmd.op) {
      result = (state.cmd.op == yank || state.cmd.op == visualLine)
                   ? exitOp
                   : exitMotion;
  ...
  if (!posLin || posLin == h || !h)
    ...
    sprintf(posBuffer, " % 3d%c  ",
            min(100, max(0, (int)(.5 + posLin * 100. / h))), '%');
  if ((overlay || overlay != prevOverlay) && term.col > 9 && term.row > 4) {
    if (!term.dirty[term.row - 1])
      xdrawline(term.line[term.row - 1], term.col * 2 / 3, term.row - 1,
                term.col - 1);
    if (!term.dirty[term.row - 2])
      xdrawline(term.line[term.row - 2], term.col * 2 / 3, term.row - 2,
                term.col - 1);
  }
  if (result == finish)
    altToggle = 0;
  ...
  if (overlay && term.col > 9 && term.row > 4 && (x > (2 * term.col / 3)) &&
    ...
    *g = (y == term.row - 2) ? styleSearch : styleCmd;
    if (y == term.row - 2)
      ...
      getChar(size(&cCmd) ? &cCmd : &lCmd, g, term.row - 1, term.col - 7,
              term.col / 3 - 6, x);
