...
#include "lib/wld/wayland.h"
#include "lib/wld/wld.h"
#include <stdbool.h>
...
static void xdgtoplevelconfigure(void *, struct xdg_toplevel *, int32_t,
...
static struct xdg_toplevel_listener xdgtoplevellistener = {xdgtoplevelconfigure,
                                                           xdgtoplevelclose};
...
void clipcopy(const Arg *dummy) { setsel(getsel(), wl.globalserial); }

void clippaste(const Arg *dummy) { wlselpaste(); }

void selpaste(const Arg *dummy) { wlselpaste(); }

void xsetsel(char *buf) { setsel(buf, wl.globalserial); }
...
void kbdkeymap(void *data, struct wl_keyboard *keyboard, uint32_t format,
  ...
  compose_table = xkb_compose_table_new_from_locale(
      wl.xkb.ctx, getenv("LANG"), XKB_COMPOSE_COMPILE_NO_FLAGS);
  if (compose_table) {
    ...
    fprintf(stderr,
            "could not create XKB compose table for locale '%s'.  "
            "Disabiling compose\n",
            getenv("LANG"));
  }
...
static xkb_keysym_t processKey(Wayland wl, xkb_keysym_t sym) {
  if (!wl.xkb.compose_state)
    return sym;
  if (sym == XKB_KEY_NoSymbol)
    return sym;
  if (xkb_compose_state_feed(wl.xkb.compose_state, sym) !=
...
void wlresize(int col, int row) {
  ...
  wld.buffer = wld_create_buffer(wld.ctx, win.w, win.h, WLD_FORMAT_XRGB8888, 0);
...
void xdgtoplevelconfigure(void *data, struct xdg_toplevel *toplevel, int32_t w,
...
void wlneeddraw(void) { wl.needdraw = true; }
...
void xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og) {
  ...
  if (IS_SET(MODE_FOCUSED)) {
    ...
    case 4: /* Steady Underline */
      wld_fill_rectangle(wld.renderer, drawcol, borderpx + cx * win.cw,
    ...
    wld_fill_rectangle(wld.renderer, drawcol, borderpx + (cx + 1) * win.cw - 1,
                       borderpx + cy * win.ch, 1, win.ch - 1);
    wld_fill_rectangle(wld.renderer, drawcol, borderpx + cx * win.cw,
                       borderpx + (cy + 1) * win.ch - 1, win.cw, 1);
  }
  wl_surface_damage(wl.surface, borderpx + cx * win.cw, borderpx + cy * win.ch,
                    win.cw, win.ch);
}
